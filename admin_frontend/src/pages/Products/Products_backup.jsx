...
export default function Products() {
  ...
  const getFilteredProducts = useCallback(() => {
    return products.filter((product) => {
      ...
      if (
        filters.material &&
        product.material_id !== parseInt(filters.material)
      )
        return false;
      if (
        filters.occasion &&
        product.occasion_id !== parseInt(filters.occasion)
      )
        return false;
  ...
  const handleDeleteProduct = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to retire this piece from the collection?",
      )
    )
      return;
    ...
    <>
      <section className="space-y-6 animate-in fade-in duration-500">
        <div className="flex items-center justify-between">
          <h2 className="brand-font text-xl font-bold text-[#800020]">
            Product Management
          </h2>
          <button
            onClick={toggleModal}
            ...
            <tbody className="text-xs divide-y divide-[#D4AF37]/5">
              {loading ? (
                ...
                products.map((product) => (
                  <tr
                    key={product.id}
                    className="hover:bg-[#FAF8F6]/50 transition-colors"
                  >
                    <td className="px-6 py-4 font-mono text-gray-500">
                      #{product.id}
                    </td>
                    <td className="px-6 py-4 font-bold text-[#4A3F35]">
                      {product.name}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <img
                        ...
                        alt={product.name}
                        className="w-10 h-10 object-cover rounded shadow-sm mx-auto"
                      />
                    </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-[#D4AF37]/10 flex justify-between items-center bg-[#FDFCFB]">
              <h3 className="brand-font text-xl font-bold text-[#800020]">
              ...
              >
                ...
                <div className="flex items-center gap-8 md:col-span-2 p-4 bg-[#FAF8F6] rounded-xl border border-[#D4AF37]/10">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label
                      className="text-[10px] font-bold uppercase text-gray-500 tracking-widest cursor-pointer"
                      htmlFor="is_featured"
                    >
                      Featured Piece
                    </label>
