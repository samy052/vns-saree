...
const Cart = () => {
  ...
  const gst = subtotal * 0.05; // 5% GST for garments
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          ...
          {cart.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-lg shadow-sm border border-[#D4AF37]/10">
              <iconify-icon icon="lucide:shopping-bag" className="text-6xl text-[#D4AF37]/30 mb-6"></iconify-icon>
              <h3 className="brand-font text-2xl text-[#800020] mb-4">Your bag is currently empty</h3>
              <p className="text-gray-500 mb-8">Begin your heritage journey by exploring our collections.</p>
            ...
            <div className="flex flex-col lg:flex-row gap-12 items-start">
              ...
              <div className="w-full lg:w-2/3 space-y-6">
                {cart.map((item, index) => (
                  <div key={item.id} className="bg-white rounded-lg p-6 shadow-sm border border-[#D4AF37]/10 flex flex-col sm:flex-row items-center gap-6 item-row transition-all duration-300 animate-slide-up" style={{ animationDelay: `${index * 0.1}s` }}>
                    ...
                    <div className="flex-grow text-center sm:text-left">
                      ...
                      <p className="text-xs text-gray-500 uppercase tracking-widest mb-4 font-semibold">
                        {item.Material?.name || 'Pure Silk'} • SKU: VNS-{item.id}
                      </p>
                    ...
                    <div className="flex flex-col items-center sm:items-end gap-4">
                      ...
                      <div className="flex items-center border border-[#D4AF37]/30 rounded-sm bg-white overflow-hidden">
                        <button 
                          ...
                          className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-[#800020] transition-colors"
                        >
                          <iconify-icon icon="lucide:minus" className="text-xs"></iconify-icon>
                        </button>
                        <input type="number" value={item.quantity} readOnly className="w-10 text-center text-sm font-bold bg-transparent outline-none border-none focus:ring-0" />
                        <button 
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-[#800020] transition-colors"
                        >
