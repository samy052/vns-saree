  ...
  return (
    <div className="relative min-h-screen bg-[#F5F1E8]" ref={rootRef}>
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        {/* Breadcrumbs */}
        <nav
          className="flex text-xs uppercase tracking-widest text-[#3D2817]/60 mb-8"
        ...
        >
          <ol className="flex items-center space-x-2">
            ...
            <li>
              <Link to="/collection" className="hover:text-[#800020]">
                Collections
              </Link>
            </li>
            <iconify-icon icon="lucide:chevron-right"></iconify-icon>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 flex flex-col md:flex-row-reverse gap-4">
            <div className="flex-1 product-3d-perspective" ref={perspectiveRef}>
              <div
                className="product-3d-frame relative bg-white rounded-xl shadow-2xl overflow-hidden border border-[#D4AF37]/20 group zoom-container"
                ref={frameRef}
              >
                <img
                  src={mainImage}
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="mb-6">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-[#800020] mb-3 leading-tight uppercase">
                {product.name}
              </h1>
              <p className="serif-text text-xl italic text-[#3D2817]/70 mb-4">
            ...
            <div className="bg-white/40 p-6 border border-[#D4AF37]/20 rounded-xl mb-8">
              <div className="flex items-baseline space-x-4 mb-1">
                <span className="text-4xl font-bold text-[#3D2817]">
                ...
                {product.old_price && (
                  <>
                    ...
                    <span className="text-green-600 font-bold text-sm tracking-wider uppercase">
                      SAVE ₹
                      {Number(product.old_price - product.price).toLocaleString(
                        "en-IN",
                      )}
                    </span>
                  </>
              ...
              <p className="text-xs text-gray-500 italic uppercase tracking-widest">
                Incl. of all taxes & free worldwide shipping
              </p>
            </div>
              ...
              <div className="flex flex-col md:flex-row gap-4 items-center">
                <div className="flex items-center border border-[#800020]/20 rounded-sm bg-white overflow-hidden">
                  <button
                    ...
                    className="w-12 h-14 flex items-center justify-center hover:bg-[#800020] hover:text-white transition-colors"
                  >
                  ...
                  <input
                    ...
                    value={quantity}
                    readOnly
                    className="w-14 h-14 text-center bg-transparent font-bold focus:ring-0 border-none"
                  />
                  <button
            ...
            <div className="border-t border-[#D4AF37]/20">
              ...
              {[
                {
                  id: "story",
                  title: "The Weave Story",
                  content:
                    "Woven meticulously over 14 days by master artisan Shri Ramdas, this saree features the rare 'Kimkhab' technique. Every flower in the border is hand-brocaded with pure mulberry silk and tested zari, ensuring a drape that feels like a second skin yet looks like royalty.",
                },
                {
                  id: "material",
                  title: "Material & Care",
                  content:
                    "100% Katan Silk. Dry Clean Only. approx. 950g. 6.5 Metres includes 80cm blouse piece.",
                },
                {
                  id: "shipping",
                  title: "Shipping & Returns",
                  content:
                    "Ships within 48 hours. Complimentary express delivery globally. We offer a 7-day easy return policy.",
                },
              ].map((item) => (
                <div
                  key={item.id}
                  className={`border-b border-[#D4AF37]/20 py-4 ${activeAccordion === item.id ? "active" : ""}`}
                >
                  <button
                    onClick={() => toggleAccordion(item.id)}
                    className="w-full flex items-center justify-between text-left"
                  >
                    <span className="brand-font font-bold text-lg tracking-wider uppercase">
                      {item.title}
                    </span>
                    <iconify-icon
                  ...
                  <div
                    className={`accordion-content overflow-hidden transition-all duration-400 ${activeAccordion === item.id ? "max-h-[500px] mt-4" : "max-h-0"}`}
                  >
            ...
            <div className="mt-8">
              <p className="text-xs font-bold uppercase tracking-widest text-[#3D2817]/60 mb-3">
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 bg-white border border-[#D4AF37]/30 rounded-sm px-4 text-sm focus:ring-[#800020] focus:border-[#800020]"
                />
                <button className="px-6 py-2 bg-[#D4AF37] text-white font-bold text-xs uppercase tracking-widest">
                  Check
                </button>
              </div>
        ...
        <section className="mt-24 reveal-up">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-bold text-[#800020] uppercase brand-font">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {products.slice(0, 4).map((p) => (
              <div
                key={p.id}
                className="group cursor-pointer bg-white p-3 rounded-xl shadow-sm border border-[#D4AF37]/10 hover:shadow-xl transition-all"
              >
                ...
                <h3 className="brand-font text-lg text-[#800020] mb-1">
                  {p.name}
                </h3>
                <p className="text-sm text-[#3D2817] font-bold">
                  ₹{Number(p.price).toLocaleString("en-IN")}
                </p>
                <Link
                  to={`/product/${p.slug}`}
                  className="mt-4 block w-full py-2 border border-[#800020] text-[#800020] text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-[#800020] hover:text-white text-center transition-all"
                >
                  VIEW PIECE
                </Link>
              </div>
